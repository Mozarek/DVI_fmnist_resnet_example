...
class FashionMNISTData(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        transform = ToTensor()
        dataset = FashionMNIST("Fashion_mnist", train=True, download=True, transform=transform)
    ...
    def val_dataloader(self):
        transform = ToTensor()
        dataset = FashionMNIST("Fashion_mnist", train=False, download=True, transform=transform)
        dataloader = DataLoader(
            dataset,
            batch_size=self.hparams.batch_size,
            num_workers=self.hparams.num_workers,
